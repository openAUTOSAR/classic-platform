	...
	*Length = 0;
		...
		&&(responseCode == DCM_E_POSITIVERESPONSE);i++)
	{
		if(PDidPtr->DDDSource[i].DDDTpyeID == DCM_DDD_SOURCE_ADDRESS)
		{
			...
			if(responseCode == DCM_E_POSITIVERESPONSE)
			{
				/* REVIEW JB 6 sep 2012: Check the return value, if FAILED or PENDING, call callout with OpStatus DCM_CANCEL and set proper NRC */
...
{
	/* REVIEW JB 6 sep 2012: Don't use local type it is not necessary and is only confusing */
	typedef struct{
	...
	Dcm_OpStatusType OpStatus = 0; /* REVIEW JB 6 sep 2012: Don't use magic number - this is INITAL */
	...
	dspReadMemoryByAddress.MemoryAddressFormat = ((uint8)(pduRxData->SduDataPtr[1])) & 0x0Fu;  /* REVIEW JB 6 sep 2012: Use mask also here (for example DCM_FORMAT_LOW_MASK) */ /*@req UDS_REQ_0x23_1 & UDS_REQ_0x23_5*/
	...
	else
	{
		...
		if(dspReadMemoryByAddress.MessageLength == (uint16)(pduRxData->SduLength))
		{
			...
			for(i = 0; i < dspReadMemoryByAddress.MemorySizeFormat; i++)
			{
				...
				dspReadMemoryByAddress.MemorySize += (uint32)(pduRxData->SduDataPtr[2 /* REVIEW JB 6 sep 2012: <---- Please use SID_AND_ALFID_LEN2 here as well!!!! */ + i + dspReadMemoryByAddress.MemoryAddressFormat]);
			}
...
{
	/* REVIEW JB 6 sep 2012: Remove internal type (see read) */
	typedef struct{
	...
	else
	{
		...
		for(i = 0; i < dspUdsWriteMemory.MemorySizeFormat; i++)
		{
			...
			dspUdsWriteMemory.MemorySize += (uint32)(pduRxData->SduDataPtr[2 + i +dspUdsWriteMemory.MemoryAddressFormat]); /* REVIEW JB 6 sep 2012: Please check all 2s - should not be magic number */
		}
...
static Dcm_NegativeResponseCodeType dynamicallyDefineDataIdentifierbyAddress(uint16 DDIdentifier,const PduInfoType *pduRxData,PduInfoType *pduTxData)
{
	...
	else
	{
		...
		/* REVIEW JB 6 sep 2012: See above */
		if(SourceLength >= DCM_MAX_DDDSOURCE_NUMBER)
		{
			responseCode = DCM_E_REQUESTOUTOFRANGE;
		}
	...
	if(responseCode == DCM_E_POSITIVERESPONSE)
	{
		...
		{
			/* REVIEW JB 6 sep 2012: Remove the magic number */
			if((Length != 0)&&( Length * (AddressFormat + MemorySizeFormat) == (pduRxData->SduLength - 5) ))
			{
				for(LengthCount = 0; (LengthCount < Length) && (responseCode == DCM_E_POSITIVERESPONSE); LengthCount++)
				{
					...
					for(SourceCount = 0; SourceCount < AddressFormat; SourceCount++)
					{
						...
						/* REVIEW JB 6 sep 2012: Magic number 5 */
				...
				if(responseCode == DCM_E_POSITIVERESPONSE)
				{
					/* REVIEW JB 6 sep 2012: Move this to the end of the function */
					DDid->DynamicallyDid = DDIdentifier;
				}
			}
			else
...
/* REVIEW JB 6 sep 2012: Reviewed until here */
...
/* REVIEW JB 7 sep 2012: SpellingError ShortTeam */
...
void DspIOControlByDataIdentifier(const PduInfoType *pduRxData,PduInfoType *pduTxData)
{
	...
	if(pduRxData->SduLength > 3)
	{
		...
		{
			/* REVIEW JB 7 sep 2012: Remove this check, because we will use callbacks (to Rte) also when use ports is selected */
