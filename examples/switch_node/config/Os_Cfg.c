...
uint8_t stack_ReadSwitches[PRIO_STACK_SIZE];
...
// ##################################    TASKS     #################################
	...
	{
		.pid = TASK_ID_ReadSwitches,
		.name = "ReadSwitches",
		.entry = ReadSwitches,
		.prio = 10,
		.proc_type = PROC_BASIC,
		.stack.size = sizeof stack_ReadSwitches,
		.stack.top = stack_ReadSwitches,
		.autostart = false,
		.application_id = APPLICATION_ID_application_1,
	},
	{
