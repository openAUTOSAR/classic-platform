...
void btask_3( void ) {
	...
	WdgM_UpdateAliveCounter (WDGM_ALIVE_BTASK3);
...
void etask_1( void ) {
	...
	for(;;) {
		WdgM_UpdateAliveCounter (WDGM_ALIVE_ETASK1);
 ...
 */
static Dio_LevelType level;

void etask_2( void ) {
    TickType wdCnt;
	LDEBUG_PRINTF("etask_2 start\n");

	WdgM_SetMode(WDGM_WIDE_MODE);
	...
	for(;;) {
		...
		ActivateTask(TASK_ID_btask_3);
		{
		    ...
		    level ^= 1;
			Dio_WriteChannel(DIO_CHANNEL_NAME_HeartBeatLED, level);

			{
				static uint8 changeMode = 1;
				if (changeMode)
				{
					WdgM_SetMode(WDGM_NARROW_MODE);
					changeMode = 0;
				}
			}
			/* WdgM supervised task. */
			GetCounterValue(COUNTER_ID_WDFailureCounter, &wdCnt);
			if (wdCnt < 75)
			{
			    IncrementCounter(COUNTER_ID_WDFailureCounter);
				WdgM_UpdateAliveCounter (WDGM_ALIVE_ETASK2);
			}
		}
	}
}
