...
#define FLS_VALIDATE_PARAM_ADDRESS_SECTOR_W_RV(_addr, _api, _rv)\
  ...
  if (1!=addrOk){\
  Det_ReportError(FLS_MODULE_ID,0,_api,FLS_E_PARAM_ADDRESS ); \
  return _rv; \
  }
...
#define FLS_VALIDATE_PARAM_LENGTH_PAGE_W_RV(_addr, _length, _api, _rv)\
  ...
  if (!lengthOk){\
    Det_ReportError(FLS_MODULE_ID,0,_api,FLS_E_PARAM_LENGTH ); \
    return _rv; \
  }
...
#define FLS_VALIDATE_STATUS_UNINIT_W_RV(_status, _api, _rv)\
  if (MEMIF_UNINIT == _status){\
    Det_ReportError(FLS_MODULE_ID,0,_api,FLS_E_UNINIT); \
    return _rv; \
  }

#define FLS_VALIDATE_STATUS_BUSY(_status, _api)\
  if (MEMIF_BUSY == _status){\
    Det_ReportError(FLS_MODULE_ID,0,_api,FLS_E_BUSY); \
    return; \
  }
...
#define FLS_VALIDATE_PARAM_DATA_W_RV(_ptr,_api, _rv) \
  if( (_ptr)==((void *)0)) { \
    Det_ReportError(FLS_MODULE_ID,0,_api,FLS_E_PARAM_DATA); \
    return _rv; \
  }
#else
  #define FLS_VALIDATE_PARAM_ADDRESS_SECTOR_W_RV(_addr, _api, _rv)
...
#define DET_REPORTERROR(_x,_y,_z,_q) Det_ReportError(FLS_MODULE_ID, _y, _z, _q)
#else
...
#define VALIDATE(_exp,_api,_err ) \
        if( !(_exp) ) { \
          Det_ReportError(FLS_MODULE_ID,0,_api,_err); \
          return; \
        }
#endif
...
static Std_VersionInfoType Fls_SST25XX_VersionInfo = {
    .vendorID 			= (uint16)FLS_VENDOR_ID,
    .moduleID 			= (uint16) FLS_MODULE_ID,
    /* Vendor numbers */
...
typedef struct {
  ...
  Spi_DataBufferType ebFlsAddr[ADDR_LENGTH];
 ...
 *
 */
static void Spi_ConvertToSpiAddr(Spi_DataBufferType *spiAddr, Fls_AddressType addr ) {
...
void Fls_SST25xx_Init( const Fls_ConfigType* ConfigPtr ){
  ...
  Spi_DataBufferType data = 0;
  int timer = 0;
  Std_ReturnType rv = E_OK;
  Spi_DataBufferType jedecId[3];
  ...
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_CMD,  &Fls_SST25xx_Global.ebCmd,NULL,sizeof(Fls_SST25xx_Global.ebCmd)/sizeof(Fls_SST25xx_Global.ebCmd));
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_ADDR,  Fls_SST25xx_Global.ebFlsAddr,NULL,sizeof(Fls_SST25xx_Global.ebFlsAddr)/sizeof(Fls_SST25xx_Global.ebFlsAddr[0]));
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_WREN,  NULL,NULL,1);
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_WRDI,  NULL,NULL,1);
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_WRSR,  NULL,NULL,1);

  /* Check that the JEDEC ID can be read */
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_DATA, NULL ,jedecId,3);

  Fls_SST25xx_Global.ebCmd = FLASH_JEDEC_ID;
  Spi_SyncTransmit(SpiConf_SpiSequence_SEQ_FLASH_CMD_DATA );
  ...
  Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_DATA, &data, NULL,1);
  rv = Spi_SyncTransmit(SpiConf_SpiSequence_SEQ_FLASH_WRSR);

    // Busy wait
    Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_DATA, NULL, &Fls_SST25xx_Global.ebReadStatus, 1);
    do {
        Fls_SST25xx_Global.ebCmd = FLASH_RDSR;
        Spi_SyncTransmit(SpiConf_SpiSequence_SEQ_FLASH_CMD2);
        timer++;
  ...
  Fls_SST25xx_Global.job.currSeq = SpiConf_SpiSequence_SEQ_FLASH_WRSR;
                           ...
                           Fls_LengthType Length )
{
  ...
  job->initialOp = true;
  job->currSeq = SpiConf_SpiSequence_SEQ_FLASH_READ;
...
Std_ReturnType Fls_SST25xx_Erase( Fls_AddressType   TargetAddress, Fls_LengthType    Length ){
  ...
  job->initialOp = true;
  job->currSeq = SpiConf_SpiSequence_SEQ_FLASH_WRITE;
...
void Fls_SST25xx_Cancel( void ){
  ...
  if (MEMIF_JOB_PENDING==Fls_SST25xx_Global.jobResultType) {
      Fls_SST25xx_Global.jobResultType=MEMIF_JOB_CANCELED;
  }
...
static Spi_SeqResultType Fls_SST25xx_ProcessJob( Fls_SST25xx_JobInfoType *job ) {
  ...
  do {
        ...
        case JOB_READ_STATUS:
            ...
            Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_DATA, NULL, &Fls_SST25xx_Global.ebReadStatus, 1);
            Fls_SST25xx_Global.ebCmd = FLASH_RDSR;
            if( SPI_TRANSMIT_FUNC(SpiConf_SpiSequence_SEQ_FLASH_CMD2,job ) != E_OK ) {
                ASSERT(0);
            }
        ...
        case JOB_MAIN:
            if( job->left != 0 ) {
                ...
                case FLS_SST25XX_ERASE:
                    ...
                    SPI_TRANSMIT_FUNC(SpiConf_SpiSequence_SEQ_FLASH_ERASE,job );
                    break;
                ...
                case FLS_SST25XX_COMPARE:
                  DEBUG(DEBUG_LOW,"%s: READ s:%04x d:%04x l:%04x\n",MODULE_NAME,job->flsAddr, job->targetAddr, job->left);
                    ...
                    Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_DATA, NULL ,job->targetAddr,job->chunkSize);
                    SPI_TRANSMIT_FUNC(SpiConf_SpiSequence_SEQ_FLASH_READ,job );
                    break;
                ...
                case FLS_SST25XX_WRITE:
                    ...
                    Spi_ConvertToSpiAddr(Fls_SST25xx_Global.ebFlsAddr,job->flsAddr);
                    Spi_SetupEB( SpiConf_SpiChannel_CH_FLASH_DATA, job->targetAddr, NULL, job->chunkSize);
                    SPI_TRANSMIT_FUNC(SpiConf_SpiSequence_SEQ_FLASH_WRITE,job );
                    break;
  ...
  if( Fls_SST25xx_Global.jobResultType == MEMIF_JOB_PENDING ) {
    ...
    case FLS_SST25XX_WRITE:
        ...
        // Error
          ...
          case FLS_SST25XX_ERASE:
#if defined(USE_DEM)
                    Dem_ReportErrorStatus(DemConf_DemEventParameter_FLS_E_ERASED_FAILED, DEM_EVENT_STATUS_FAILED);
#endif
                    break;
          case FLS_SST25XX_READ:
#if defined(USE_DEM)
