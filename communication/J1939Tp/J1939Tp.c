...
void J1939Tp_Init(const J1939Tp_ConfigType* ConfigPtr) {
	...
	for (int i = 0; i < J1939TP_CHANNEL_COUNT; i++) {
		if (ConfigPtr->Channels[i].Direction == J1939TP_TX) {
			...
			channelInfos[i].TxState = &(txChannelInfos[i]);
			...
			channelInfos[i].RxState = &(rxChannelInfos[i]);
...
static inline void J1939Tp_Internal_RxIndication_Cm(PduInfoType* PduInfoPtr, J1939Tp_Internal_ChannelInfoType* ChannelInfoPtr) {
	...
	J1939Tp_ProtocolType protocol = J1939Tp_Internal_GetPf(pf);
	...
	if (Command != BAM_CONTROL_VALUE || Command != RTS_CONTROL_VALUE) {
		return;
	}
	...
	ChannelInfoPtr->RxState->TotalDtToReceiveCount = PduInfoPtr->SduDataPtr[BAM_RTS_BYTE_NUM_PACKETS];
...
static inline uint16 J1939Tp_Internal_GetRtsMessageSize(PduInfoType* pduInfo) {
	return (((uint16)pduInfo->SduDataPtr[BAM_RTS_BYTE_LENGTH_1]) << 8) | pduInfo->SduDataPtr[BAM_RTS_BYTE_LENGTH_1];
}
...
static inline boolean J1939Tp_Internal_LastDtSent(J1939Tp_Internal_TxChannelInfoType* TxChannelState) {
	return J1939TP_Internal_GetNumDtPacketsToSend(TxChannelState->TotalMessageSize) == TxChannelState->TotalSentDtCount;
}
...
static inline void J1939Tp_Internal_SendRts(J1939Tp_Internal_ChannelInfoType* ChannelInfoPtr, const PduInfoType* TxInfoPtr) {
	...
	cmRtsData[BAM_RTS_BYTE_LENGTH_1] = (uint8)(TxInfoPtr->SduLength & 0x00FF);
	cmRtsData[BAM_RTS_BYTE_LENGTH_2] = (uint8)(TxInfoPtr->SduLength & 0xFF00);
...
static inline void J1939Tp_Internal_SendCts(J1939Tp_Internal_ChannelInfoType* ChannelInfoPtr, PduInfoType* RtsPduInfoPtr) {
	...
	ctsData[CTS_BYTE_PGN_1] = RtsPduInfoPtr->SduDataPtr[BAM_RTS_BYTE_PGN_1];
	ctsData[CTS_BYTE_PGN_2] = RtsPduInfoPtr->SduDataPtr[BAM_RTS_BYTE_PGN_2];
	ctsData[CTS_BYTE_PGN_3] = RtsPduInfoPtr->SduDataPtr[BAM_RTS_BYTE_PGN_3];
