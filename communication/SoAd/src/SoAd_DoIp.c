...
static void createAndSendNack(uint16 sockNr, uint8 nackCode)
{
    ...
    if ( bytesSent != 8+1) {
        ...
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_CREATE_AND_SEND_NACK_ID, SOAD_E_UNEXPECTED_EXECUTION);
    }
}
...
static void createAndSendDiagnosticAck(uint16 sockNr, uint16 sa, uint16 ta)
{
    ...
    if (bytesSent != 8+5) {
        // Failed to send ack. Link is probably down.
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_CREATE_AND_SEND_D_ACK_ID, SOAD_E_UNEXPECTED_EXECUTION);
    }
}
...
static void createAndSendDiagnosticNack(uint16 sockNr, uint16 sa, uint16 ta, uint8 nackCode)
{
    ...
    if (bytesSent != 8+5) {
        // Failed to send ack. Link is probably down.
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_CREATE_AND_SEND_D_NACK_ID, SOAD_E_UNEXPECTED_EXECUTION);
    }
...
void DoIp_SendVehicleAnnouncement(uint16 sockNr) {
    ...
    if (SoAd_BufferGet(8+33, &txBuffer)) {
        ...
        if (bytesSent != 8+33) {
            // Failed to send ack. Link is probably down.
            DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_VEHICLE_ID_REQ_ID, SOAD_E_UNEXPECTED_EXECUTION);
        }
        SoAd_BufferFree(txBuffer);

    } else {
        // Not enough buffers. Report error in error log.
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_VEHICLE_ID_REQ_ID, SOAD_E_NOBUFS);
    }
}
...
static void createAndSendAliveCheck(uint16 connectionIndex) {
    ...
    if (SoAd_BufferGet(8, &txBuffer)) {
        ...
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_CREATE_AND_SEND_ALIVE_CHECK_ID, SOAD_E_NOBUFS);
    }
    ...
    if ((7 == payloadLength) || (11 == payloadLength) || (10 == payloadLength/*NOTE: REMOVE WHEN MOVED TO CANOE8.1*/)) {
        ...
        if (SoAd_BufferGet(8+13, &txBuffer)) {
                ...
                if (TRUE == isRoutingTypeSupported(activationType)) {
                    ...
                    if (DOIP_SOCKET_ASSIGNMENT_SUCCESSFUL == socketHandleResult) {
                        ...
                        default:
                            ...
                            DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_ROUTING_ACTIVATION_REQ_ID, SOAD_E_SHALL_NOT_HAPPEN);
                            break;
                        }
            ...
            if (bytesSent != 8+13) {
                ...
                DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_ROUTING_ACTIVATION_REQ_ID, SOAD_E_UNEXPECTED_EXECUTION);
            }
            SoAd_BufferFree(txBuffer);
            ...
            DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_ROUTING_ACTIVATION_REQ_ID, SOAD_E_NOBUFS);
    ...
    if (0 == payloadLength) {
        ...
        if (SoAd_BufferGet(8+7, &txBuffer)) {
            ...
            if (bytesSent != 8+7) {
                 ...
                 */
                DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_ENTITY_STATUS_REQ_ID, SOAD_E_UNEXPECTED_EXECUTION);
            }
            SoAd_BufferFree(txBuffer);

        } else {
            DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_ENTITY_STATUS_REQ_ID, SOAD_E_NOBUFS);
            createAndSendNack(sockNr, DOIP_E_OUT_OF_MEMORY);
        ...
        if (0 == remainingConnections) {
            ...
            if ((pendingRoutingActivationSocket != 0xff) && (NULL != pendingRoutingActivationTxBuffer)) {
                ...
                if (bytesSent != (8+13)) {
                    ...
                    DET_REPORTERROR(SOAD_MODULE_ID, 0, DOIP_HANDLE_ALIVECHECK_RESP, SOAD_E_UNEXPECTED_EXECUTION);
                }
...
static void handleTimeout(uint16 connectionIndex) {
    ...
    if ((pendingRoutingActivationSocket != 0xff) && (NULL != pendingRoutingActivationTxBuffer)) {
        ...
        if (bytesSent != (8+13)) {
            ...
            DET_REPORTERROR(SOAD_MODULE_ID, 0, DOIP_HANDLE_ALIVECHECK_TIMEOUT, SOAD_E_UNEXPECTED_EXECUTION);
        }
        SoAd_BufferFree(pendingRoutingActivationTxBuffer);
        pendingRoutingActivationTxBuffer = NULL;
        pendingRoutingActivationSocket = 0xff;
        ...
        if (lookupResult == LOOKUP_SA_TA_OK) {
            ...
            if (SoAd_BufferGet(SOAD_RX_BUFFER_SIZE, &pduInfo.SduDataPtr)) {
                ...
                if (result == BUFREQ_OK && len > 0) {
                    ...
                    DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_DIAG_MSG_ID, SOAD_E_SHALL_NOT_HAPPEN);
                }
...
void DoIp_HandleTcpRx(uint16 sockNr)
{
    ...
    if (SoAd_BufferGet(SOAD_RX_BUFFER_SIZE, &rxBuffer)) {
        ...
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_TCP_RX_ID, SOAD_E_NOBUFS);
    }
}
...
void DoIp_HandleUdpRx(uint16 sockNr)
{
    ...
    if (SoAd_BufferGet(SOAD_RX_BUFFER_SIZE, &rxBuffer)) {
        ...
        // No rx buffer available. Report this in Det. Message should be handled in the next (scanSockets) loop.
        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_UDP_RX_ID, SOAD_E_NOBUFS);
    }
}
            ...
            || (SocketAdminList[socketNr].SocketState == SOCKET_UDP_READY))
        {
                ...
                if(SoAd_BufferGet(txPduInfo.SduLength, &txPduInfo.SduDataPtr))
                {
                    ...
                    if (bytesSent == txPduInfo.SduLength) {
                        ...
                        SoAd_BufferFree(txPduInfo.SduDataPtr);
                        DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_TP_TRANSMIT_ID, SOAD_E_UNEXPECTED_EXECUTION);
                    ...
                    DET_REPORTERROR(SOAD_MODULE_ID, 0, SOAD_DOIP_HANDLE_TP_TRANSMIT_ID, SOAD_E_NOBUFS);
